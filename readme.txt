...
3. Initializing CI Server: Launches the CI server script (`2_ci_server.sh`) in the background. The CI server monitors the staging environment's status, waiting for an HTTP status code of 200. It then makes HTTP requests to specific URLs (`http://192.168.33.9/gitlab`, development environment, etc.).
